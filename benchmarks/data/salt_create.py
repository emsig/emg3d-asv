...
np.savez_compressed('./salt_data.npz', res=cres, mesh=save_mesh)
...
grid.plot_3d_slicer(
        cmodels['res'], zslice=-2000,
        clim=np.log10([np.nanmin(model.res), 50]))
...
pfield = emg3d.solver.solver(
        ...
        linerelaxation=True, verb=3)

grid.plot_3d_slicer(
        pfield, zslice=-2000, view='abs', vType='Ex', clim=[1e-16, 1e-10],
